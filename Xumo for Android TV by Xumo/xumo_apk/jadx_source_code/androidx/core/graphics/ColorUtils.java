...
public final class ColorUtils {
    ...
    public static double calculateContrast(@ColorInt int i, @ColorInt int i2) {
        if (Color.alpha(i2) == 255) {
            ...
            double calculateLuminance = calculateLuminance(i) + VideoCastManager.DEFAULT_VOLUME_STEP;
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull float[] fArr) {
        ...
        float f2 = max - min;
        float f3 = (max + min) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
        if (max == min) {
            i = 0;
            f = 0;
        } else {
            i = max == i ? ((i2 - i3) / f2) % 1086324736 : max == i2 ? ((i3 - i) / f2) + 1073741824 : ((i - i2) / f2) + 1082130432;
            f = f2 / (1065353216 - Math.abs((ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * f3) - 1.0f));
        }
    ...
    public static int HSLToColor(@NonNull float[] fArr) {
        ...
        float f2 = fArr[1];
        fArr = fArr[2];
        float abs = (1.0f - Math.abs((fArr * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) - 1.0f)) * f2;
        fArr -= 0.5f * abs;
        float abs2 = (1.0f - Math.abs(((f / 60.0f) % ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) - 1.0f)) * abs;
        switch (((int) f) / 60) {
            case 0:
    ...
    public static double distanceEuclidean(@NonNull double[] dArr, @NonNull double[] dArr2) {
        return Math.sqrt((Math.pow(dArr[0] - dArr2[0], MediaLoadOptions.PLAYBACK_RATE_MAX) + Math.pow(dArr[1] - dArr2[1], MediaLoadOptions.PLAYBACK_RATE_MAX)) + Math.pow(dArr[2] - dArr2[2], MediaLoadOptions.PLAYBACK_RATE_MAX));
    }
