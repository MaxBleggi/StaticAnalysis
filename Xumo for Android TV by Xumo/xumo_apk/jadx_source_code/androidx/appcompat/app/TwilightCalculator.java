...
class TwilightCalculator {
    ...
    public void calculateTwilight(long j, double d, double d2) {
        ...
        Double.isNaN(d3);
        sin = ((((sin + d3) + (Math.sin((double) (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT * f2)) * 3.4906598739326E-4d)) + (Math.sin((double) (f2 * 3.0f)) * 5.236000106378924E-6d)) + 1.796593063d) + 3.141592653589793d;
        double d4 = (-d2) / 360.0d;
        double d5 = (double) (f - J0);
        Double.isNaN(d5);
        double round = (double) (((float) Math.round(d5 - d4)) + J0);
        Double.isNaN(round);
        round = ((round + d4) + (Math.sin(d3) * 0.0053d)) + (Math.sin(MediaLoadOptions.PLAYBACK_RATE_MAX * sin) * -0.0069d);
