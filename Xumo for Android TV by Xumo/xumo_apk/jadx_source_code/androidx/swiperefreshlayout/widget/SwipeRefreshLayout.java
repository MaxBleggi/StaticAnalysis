...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    private void moveSpinner(float f) {
        ...
        double pow = Math.pow(max2, MediaLoadOptions.PLAYBACK_RATE_MAX);
        Double.isNaN(max2);
        abs = ((float) (max2 - pow)) * 2.0f;
        int i2 = this.mOriginalOffsetTop + ((int) ((f2 * min) + ((f2 * abs) * 2.0f)));
        ...
        this.mProgress.setProgressRotation((((max * 0.4f) - 16.0f) + (abs * 2.0f)) * DRAG_RATE);
