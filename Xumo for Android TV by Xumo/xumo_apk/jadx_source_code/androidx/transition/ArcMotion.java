...
public class ArcMotion extends PathMotion {
    ...
    private static float toTangent(float f) {
        if (f >= 0.0f && f <= 90.0f) {
            return (float) Math.tan(Math.toRadians((double) (f / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)));
        }
    ...
    public Path getPath(float f, float f2, float f3, float f4) {
        ...
        float f8 = (f + f3) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
        float f9 = (f2 + f4) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
        float f10 = 0.25f * f7;
        Object obj = f2 > f4 ? 1 : null;
        if (Math.abs(f5) < Math.abs(f6)) {
            f5 = Math.abs(f7 / (f6 * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT));
            if (obj != null) {
                f6 = f5 + f4;
                f5 = f3;
            } else {
            ...
            f7 /= f5 * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
            if (obj != null) {
                f6 = f2;
                f5 = f7 + f;
            } else {
        ...
        path.cubicTo((f + f5) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT, (f2 + f6) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT, (f5 + f3) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT, (f6 + f4) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT, f3, f4);
        return path;
    }
}
