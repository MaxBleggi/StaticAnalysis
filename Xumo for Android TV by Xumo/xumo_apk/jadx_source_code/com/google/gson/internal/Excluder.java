...
public final class Excluder implements TypeAdapterFactory, Cloneable {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> typeToken) {
        ...
        boolean z2;
        final Gson gson2;
        final TypeToken<T> typeToken2;
        Class rawType = typeToken.getRawType();
        boolean excludeClassChecks = excludeClassChecks(rawType);
        if (!excludeClassChecks) {
            if (!excludeClassInStrategy(rawType, true)) {
                z = false;
                if (!excludeClassChecks) {
                    if (excludeClassInStrategy(rawType, false)) {
                        z2 = false;
                        if (z && !z2) {
                            return null;
                        }
...
*/
    ...
    private boolean excludeClassChecks(Class<?> cls) {
        ...
        if ((this.serializeInnerClasses || !isInnerClass(cls)) && isAnonymousOrLocal(cls) == null) {
            return null;
        }
        return true;
    }
    ...
    public boolean excludeClass(Class<?> cls, boolean z) {
        if (!excludeClassChecks(cls)) {
            if (excludeClassInStrategy(cls, z) == null) {
                return null;
            }
        }
        return true;
