...
public class ShadowDrawableWrapper extends DrawableWrapper {
    ...
    private void drawShadow(Canvas canvas) {
        ...
        float f6 = f5 * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
        ...
        float f9 = f5 / ((r0.rawShadowSize - (r0.rawShadowSize * 1.0f)) + f5);
    ...
    private void buildShadowCorners() {
        ...
        if (f > 0.0f) {
            ...
            float f3 = ((1.0f - f2) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + f2;
            Paint paint = r0.cornerShadowPaint;
            Shader shader = r8;
            Shader radialGradient = new RadialGradient(0.0f, 0.0f, f, new int[]{0, r0.shadowStartColor, r0.shadowMiddleColor, r0.shadowEndColor}, new float[]{0.0f, f2, f3, 1.0f}, TileMode.CLAMP);
            paint.setShader(shader);
        }
        r0.edgeShadowPaint.setShader(new LinearGradient(0.0f, rectF.top, 0.0f, rectF2.top, new int[]{r0.shadowStartColor, r0.shadowMiddleColor, r0.shadowEndColor}, new float[]{0.0f, SHADOW_HORIZ_SCALE, 1.0f}, TileMode.CLAMP));
    ...
    public float getMinWidth() {
        return (Math.max(this.rawMaxShadowSize, this.cornerRadius + (this.rawMaxShadowSize / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)) * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + (this.rawMaxShadowSize * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
    }

    public float getMinHeight() {
        return (Math.max(this.rawMaxShadowSize, this.cornerRadius + ((this.rawMaxShadowSize * SHADOW_MULTIPLIER) / ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT)) * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) + ((this.rawMaxShadowSize * SHADOW_MULTIPLIER) * ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT);
    }
}
