...
public final class SsaDecoder extends SimpleSubtitleDecoder {
    ...
    private void parseFormatLine(String str) {
        ...
        for (int i = 0; i < this.formatKeyCount; i++) {
            ...
            if (hashCode != 100571) {
                if (hashCode != 3556653) {
                    if (hashCode == 109757538) {
                        if (toLowerInvariant.equals("start")) {
                            obj = null;
                            switch (obj) {
                                case null:
            ...
            } else if (toLowerInvariant.equals("end")) {
                obj = 1;
                switch (obj) {
                    case null:
                        this.formatStartIndex = i;
    ...
    public static long parseTimecodeUs(String str) {
        ...
        if (str.matches()) {
            return (((((Long.parseLong(str.group(1)) * 60) * 60) * 1000000) + ((Long.parseLong(str.group(2)) * 60) * 1000000)) + (Long.parseLong(str.group(3)) * 1000000)) + (Long.parseLong(str.group(4)) * NotificationOptions.SKIP_STEP_TEN_SECONDS_IN_MS);
        }
        return C.TIME_UNSET;
    }
}
