...
*/
    ...
    private static final class TrueHdSampleRechunker {
        ...
        public void startSample(ExtractorInput extractorInput, int i, int i2) throws IOException, InterruptedException {
            if (!this.foundSyncframe) {
                ...
                if (Ac3Util.parseTrueHdSyncframeAudioSampleCount(this.syncframePrefix) != null) {
    ...
    private final class InnerEbmlReaderOutput implements EbmlReaderOutput {
        ...
        public boolean isLevel1Element(int i) {
            if (!(i == 357149030 || i == MatroskaExtractor.ID_CLUSTER || i == MatroskaExtractor.ID_CUES)) {
                if (i != MatroskaExtractor.ID_TRACKS) {
                    return false;
                }
            }
            return true;
        ...
        if (i == ID_BLOCK_GROUP) {
            ...
            if (i == ID_SEGMENT) {
                if (this.segmentContentPosition != -1) {
                    if (this.segmentContentPosition != j) {
            ...
            if (i == ID_TRACK_ENTRY) {
                ...
                if (i != ID_CONTENT_ENCODING) {
                    if (i != ID_CONTENT_ENCODINGS) {
                        ...
                        if (this.currentTrack.sampleStrippedBytes != 0) {
    ...
    void stringElement(int i, String str) throws ParserException {
        ...
        } else if (DOC_TYPE_WEBM.equals(str) != 0) {
        } else {
            if (DOC_TYPE_MATROSKA.equals(str) == 0) {
                            ...
                            if (matroskaExtractor.scratch.data[i9] != (byte) 0) {
                                ...
                                while (r14 < 8) {
                                    ...
                                    if ((matroskaExtractor.scratch.data[i9] & i10) != 0) {
                                        r10 += r14;
                                        readScratch(extractorInput2, r10);
                                        j = (long) ((matroskaExtractor.scratch.data[i9] & i8) & (i10 ^ -1));
                                        i6 = i9 + 1;
                                        while (true) {
                                            j2 = j;
                                            if (i6 >= r10) {
                                            ...
                                            j = (j2 << 8) | ((long) (matroskaExtractor.scratch.data[i6] & i8));
                                            i6++;
                                            i8 = 255;
                                        }
                                        if (r6 > 0) {
    ...
    private static void setSampleDuration(byte[] bArr, long j, String str, int i, long j2, byte[] bArr2) {
        if (j == C.TIME_UNSET) {
            ...
            j = Util.getUtf8Bytes(String.format(Locale.US, str, new Object[]{Integer.valueOf(r1), Integer.valueOf(r2), Integer.valueOf(r3), Integer.valueOf(j)}));
        }
    ...
    private SeekMap buildSeekMap() {
        if (!(this.segmentContentPosition == -1 || this.durationUs == C.TIME_UNSET || this.cueTimesUs == null || this.cueTimesUs.size() == 0 || this.cueClusterPositions == null)) {
