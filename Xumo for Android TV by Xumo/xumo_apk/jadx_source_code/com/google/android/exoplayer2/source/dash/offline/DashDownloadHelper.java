...
public final class DashDownloadHelper extends DownloadHelper {
    ...
    public DashDownloadHelper(Uri uri, Factory factory) {
        this.uri = uri;
    ...
    public TrackGroupArray getTrackGroups(int i) {
        Assertions.checkNotNull(this.manifest);
        i = this.manifest.getPeriod(i).adaptationSets;
        TrackGroup[] trackGroupArr = new TrackGroup[i.size()];
        for (int i2 = 0; i2 < trackGroupArr.length; i2++) {
            List list = ((AdaptationSet) i.get(i2)).representations;
            Format[] formatArr = new Format[list.size()];
            int size = list.size();
            for (int i3 = 0; i3 < size; i3++) {
                formatArr[i3] = ((Representation) list.get(i3)).format;
            }
