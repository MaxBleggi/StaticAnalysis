...
public class DefaultTimeBar extends View implements TimeBar {
    ...
    public DefaultTimeBar(Context context, AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            context = context.getTheme().obtainStyledAttributes(attributeSet, R.styleable.DefaultTimeBar, 0, 0);
            try {
                this.scrubberDrawable = context.getDrawable(R.styleable.DefaultTimeBar_scrubber_drawable);
                ...
                this.barHeight = context.getDimensionPixelSize(R.styleable.DefaultTimeBar_bar_height, dpToPx);
                this.touchTargetHeight = context.getDimensionPixelSize(R.styleable.DefaultTimeBar_touch_target_height, dpToPx2);
                this.adMarkerWidth = context.getDimensionPixelSize(R.styleable.DefaultTimeBar_ad_marker_width, dpToPx3);
                this.scrubberEnabledSize = context.getDimensionPixelSize(R.styleable.DefaultTimeBar_scrubber_enabled_size, dpToPx4);
                this.scrubberDisabledSize = context.getDimensionPixelSize(R.styleable.DefaultTimeBar_scrubber_disabled_size, dpToPx5);
                this.scrubberDraggedSize = context.getDimensionPixelSize(R.styleable.DefaultTimeBar_scrubber_dragged_size, dpToPx6);
                attributeSet = context.getInt(R.styleable.DefaultTimeBar_played_color, -1);
                dpToPx6 = context.getInt(R.styleable.DefaultTimeBar_scrubber_color, getDefaultScrubberColor(attributeSet));
                dpToPx3 = context.getInt(R.styleable.DefaultTimeBar_buffered_color, getDefaultBufferedColor(attributeSet));
                dpToPx = context.getInt(R.styleable.DefaultTimeBar_unplayed_color, getDefaultUnplayedColor(attributeSet));
                dpToPx2 = context.getInt(R.styleable.DefaultTimeBar_ad_marker_color, DEFAULT_AD_MARKER_COLOR);
                dpToPx4 = context.getInt(R.styleable.DefaultTimeBar_played_ad_marker_color, getDefaultPlayedAdMarkerColor(dpToPx2));
        ...
        this.stopScrubbingRunnable = new -$$Lambda$DefaultTimeBar$Qcgn0kqjCzq5x_ej2phsDpb1YTU();
        if (this.scrubberDrawable != null) {
    ...
    private void drawTimeBar(Canvas canvas) {
        ...
        height += centerY;
        if (this.duration <= 0) {
            canvas.drawRect((float) r0.progressBar.left, (float) centerY, (float) r0.progressBar.right, (float) height, r0.unplayedPaint);
            return;
        }
        int i = r0.bufferedBar.left;
        int i2 = r0.bufferedBar.right;
        int max = Math.max(Math.max(r0.progressBar.left, i2), r0.scrubberBar.right);
        if (max < r0.progressBar.right) {
            canvas.drawRect((float) max, (float) centerY, (float) r0.progressBar.right, (float) height, r0.unplayedPaint);
        }
        i = Math.max(i, r0.scrubberBar.right);
        if (i2 > i) {
            canvas.drawRect((float) i, (float) centerY, (float) i2, (float) height, r0.bufferedPaint);
        }
        if (r0.scrubberBar.width() > 0) {
            canvas.drawRect((float) r0.scrubberBar.left, (float) centerY, (float) r0.scrubberBar.right, (float) height, r0.playedPaint);
        }
        if (r0.adGroupCount != 0) {
            long[] jArr = (long[]) Assertions.checkNotNull(r0.adGroupTimesMs);
            boolean[] zArr = (boolean[]) Assertions.checkNotNull(r0.playedAdGroups);
            max = r0.adMarkerWidth / 2;
            for (int i3 = 0; i3 < r0.adGroupCount; i3++) {
                int min = r0.progressBar.left + Math.min(r0.progressBar.width() - r0.adMarkerWidth, Math.max(0, ((int) ((((long) r0.progressBar.width()) * Util.constrainValue(jArr[i3], 0, r0.duration)) / r0.duration)) - max));
                canvas.drawRect((float) min, (float) centerY, (float) (min + r0.adMarkerWidth), (float) height, zArr[i3] ? r0.playedAdMarkerPaint : r0.adMarkerPaint);
            }
        }
    }
