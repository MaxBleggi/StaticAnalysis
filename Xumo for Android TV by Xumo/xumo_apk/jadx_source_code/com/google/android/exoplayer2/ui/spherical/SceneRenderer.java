...
class SceneRenderer implements VideoFrameMetadataListener, CameraMotionListener {
    ...
    private void setProjection(@Nullable byte[] bArr, int i, long j) {
        ...
        if (i2 != this.lastStereoMode || Arrays.equals(bArr2, this.lastProjectionData) == null) {
            bArr = null;
            if (this.lastProjectionData != 0) {
                bArr = ProjectionDecoder.decode(this.lastProjectionData, this.lastStereoMode);
            }
            if (bArr == null || ProjectionRenderer.isSupported(bArr) == 0) {
                bArr = Projection.createEquirectangular(this.lastStereoMode);
            }
            this.projectionQueue.add(j, bArr);
        }
    }
}
