...
public final class AudioFocusManager {
    ...
    private class AudioFocusListener implements OnAudioFocusChangeListener {
        ...
        public void onAudioFocusChange(int i) {
            if (i != 1) {
                switch (i) {
                    case -3:
                        if (AudioFocusManager.this.willPauseWhenDucked() == 0) {
            ...
            i = AudioFocusManager.this.audioFocusState == 3 ? 1045220557 : 1065353216;
            if (AudioFocusManager.this.volumeMultiplier != i) {
                AudioFocusManager.this.volumeMultiplier = i;
                AudioFocusManager.this.playerControl.setVolumeMultiplier(i);
            }
        }
    }
        ...
        if (!Util.areEqual(this.audioAttributes, audioAttributes)) {
            ...
            if (this.focusGain != 1) {
                if (this.focusGain != null) {
                    ...
                    Assertions.checkArgument(audioAttributes, "Automatic handling of audio focus is only available for USAGE_MEDIA and USAGE_GAME.");
                    if (z && (i == 2 || i == 3)) {
    ...
    public int handlePrepare(boolean z) {
        ...
        return z ? requestAudioFocus() : true;
    }
    ...
    private void abandonAudioFocus(boolean z) {
        if (this.focusGain != 0 || this.audioFocusState != 0) {
            if (this.focusGain != 1 || this.audioFocusState == -1 || z) {
                if (Util.SDK_INT >= true) {
                    abandonAudioFocusV26();
                } else {
                    abandonAudioFocusDefault();
                }
                this.audioFocusState = false;
            }
        }
    }
