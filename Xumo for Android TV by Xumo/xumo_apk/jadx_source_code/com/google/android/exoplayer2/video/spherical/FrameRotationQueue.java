...
public final class FrameRotationQueue {
    ...
    public boolean pollRotationMatrix(float[] fArr, long j) {
        ...
        if (this.recenterMatrixComputed == null) {
    ...
    private static void getRotationMatrixFromAngleAxis(float[] fArr, float[] fArr2) {
        ...
        float f2 = -fArr2[1];
        fArr2 = -fArr2[2];
        float length = Matrix.length(f, f2, fArr2);
        if (length != 0.0f) {
            Matrix.setRotateM(fArr, 0, (float) Math.toDegrees((double) length), f / length, f2 / length, fArr2 / length);
            return;
        }
        Matrix.setIdentityM(fArr, 0);
    }
