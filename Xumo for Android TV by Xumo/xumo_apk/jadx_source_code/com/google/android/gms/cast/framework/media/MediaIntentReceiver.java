...
public class MediaIntentReceiver extends BroadcastReceiver {
    ...
    protected void onReceiveActionSkipNext(Session session) {
        if (session instanceof CastSession) {
            session = zzh((CastSession) session);
            if (session != null) {
                if (!session.isPlayingAd()) {
                    session.queueNext(null);
                }
            }
        }
    }
    ...
    private static void zza(CastSession castSession, long j) {
        if (j != 0) {
            castSession = zzh(castSession);
            if (!(castSession == null || castSession.isLiveStream())) {
                if (!castSession.isPlayingAd()) {
                    castSession.seek(castSession.getApproximateStreamPosition() + j);
                }
            }
        }
    }
