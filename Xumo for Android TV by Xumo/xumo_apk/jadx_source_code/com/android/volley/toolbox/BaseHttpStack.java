...
public abstract class BaseHttpStack implements HttpStack {
    ...
    public final HttpResponse performRequest(Request<?> request, Map<String, String> map) throws IOException, AuthFailureError {
        request = executeRequest(request, map);
        map = new BasicHttpResponse(new BasicStatusLine(new ProtocolVersion("HTTP", 1, 1), request.getStatusCode(), ""));
        List arrayList = new ArrayList();
        for (Header header : request.getHeaders()) {
            arrayList.add(new BasicHeader(header.getName(), header.getValue()));
        }
        map.setHeaders((org.apache.http.Header[]) arrayList.toArray(new org.apache.http.Header[arrayList.size()]));
        InputStream content = request.getContent();
        if (content != null) {
            HttpEntity basicHttpEntity = new BasicHttpEntity();
            basicHttpEntity.setContent(content);
            basicHttpEntity.setContentLength((long) request.getContentLength());
